  ...
  for(var i = 0; i < 5; i++) {
    if(posts < entries.length) {
        if (postbody[posts].innerText.length > 500) {
          postbody[posts].innerHTML = postbody[posts].innerHTML.slice(0, 500)+"<span>... </span><a href='javascript: void(0);' onclick='showMore(this);' class='showmore'>show more</a>"+"<span style='display: none;'>"+postbody[posts].innerHTML.slice(500, postbody[posts].innerHTML.length);//+"</span>";
